#' @title DGP Acceptance Loop
#' 
#' @description  Runs iterative acceptance loop.  
#' 
#' @details 
#' At each iteration, 8 models are trained and predict on holdout set and subsequent applicant batch
...
summary(model_glm) #Use X9 - negatively correlated with default = accept those with highest values of X9 on opening day.

### Opening Day: accept-reject applicants in Batch 1 - Accept lowest 20% risk based on annual income, reject lower 80%. ### 
acc <- 
  app %>% 
  slice_max(X9, prop = 0.20, with_ties = FALSE)
...
### - unfortunately, forgot to re-do this for DGP, uses parameter settings tested for LC. Ideally, should have tested RF parameters specifically for DGP.
...
### HCO/PAR at i=1 needs to have RF predictions, but don't have yet. Use again X9 as proxy for this function at i=0 ###
rej_ri <- rej[,-c(20)]  
acc_ri <- acc
acc_ri$rf <- acc$X9  
rej_ri$rf <- rej$X9  
...
loop <- for(i in 1:length(rapp_list)){
  ...
  rej_ri <- rbind(rej_meta[,-c(20)], rej[,-c(20)])  
