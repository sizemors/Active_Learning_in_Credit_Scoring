#' @title Reject Inference Functions for LendingClub 
#' 
...
hco_func <- function(df_acc, df_rej) { 
  ...
  #Calculate bad rate in accepts. If over 1.0, set as 1.0.
  hc = ifelse(sum(ifelse(df_acc[ ,"BAD"] == "BAD", 1, 0)) / nrow(df_acc) * t < 1.0, sum(ifelse(df_acc[ ,"BAD"] == "BAD", 1, 0)) / nrow(df_acc) * t, 1.0)
...
### Calculate bad rate in each decile of accepts and multiply by risk factor. If over 1.0, set as 1.0. ###
